...
+ EAP Types (Extensible Authentication Protocol)
    ...
    + EAP-FAST (Flexible Authentication via Secure Tunneling)
        ...
        + Based on PAC file (Protected Access Credential)
        ...
        + Three-phases process
            + Phase 0: optional, provisioning the client w/ a PAC file
                + Using EAP-MACHAPSv2 or EAP-GTC
                + Once successful, client not yet authenticated
            + Phase 1: establish TLS tunnel based on the PAC file
            + Phase 2: authenticate the client within the TLS tunnel (EAP-TLS, EAP-MSCHAPSv2, EAP-GTC)
    + EAP-FASTv2 (EAP-Chaining)
        + Tie machine authentication to user authentication
            ...
            + Perform double authentication within single EAP transaction
...
+ Authentication with Frame (Previous Call Check)
    ...
    + Verification from ISE-1: 
        ...
        + Policy > Conditions > Authentications > Compond Conditions > Wired_802.1x: Service TYpe=Framed NAS-Port-TYpe=Ethernet
...
+ Demo:
    + SW3 Config
        ...
        ```
    + ISE Config
        + Action: Policy > Results > Authorization > Authorization Profiles > Add: Name=DATA_VLAN_90; VLAN ID/Name=90 (ID = vlan id, Name = user name) > Submit
        + Rule: Policy > Authorization > Profiled Non-Cisco IP Phones > Edit: Insert New Rule Below: Name=MAB_DATA_VLAN; Condition=Select Existing Condition from Library > Condition Name=Compound conditions > Wired_MAB; Authorization Profile > Standard > Data_VLAN_90
    + SW3 Verification: 
        ```cfg
        debug radius authentication
...
+ Demo: Config ISE-1
    ...
    + Create Action: Policy > Results > Authorization > Authorization Profiles > Add: 
...
+ Demo: IP Device Tracking - R2 connected w/ VLAN 29 
    ...
    + SW1 Config:
        ...
        int f1/0/2
          ...
          no shut
        exit
        ! Observe msgs: EPN-6-AAA, EPM-6-IPEVENT, EPM-7-POLICY_APP_SUCCESS
...
+ Demo: Basic Config
    + Pre-requirement:
        ...
        2. Switch can apply authorization: `aaa authentication network` properly configured
...
+ Demo
    + SW3 Config
        ...
        ```
    + ISE:
        + Action: Policy > Policy Elements > Results > Authorization > Authorization Profiles > DACL_DATA_VLAN90
        + Format: `ip:inacl#1=permit icmp any any`
        + Action: Policy > Policy Elements > Results > Authorization > Authorization Profiles > Add: Name=PER_USER_ACL; Advanced Attribute Settings=(Cisco:cisco-av-pair=ip:inacl#1 tcp any any; Cisco:cisco-av-pair=ip:inacl#2 permit ip any any) > Submit
...
+ Demo: Troubleshooting
    ...
    + SW3 & PC-B Trigger:
        ...
        PC-B: `ifconfig /all`; Back to DHCP Config
    + SW3 Config:
        ```cfg
        conf t
        authentication port-control force-authorized
