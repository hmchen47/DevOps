...
- High Availability (Failover) Deployment
  ...
  <div style="margin: 0.5em; display: flex; justify-content: center; align-items: center; flex-flow: row wrap;">
    <a href="url" ismap target="_blank">
      <img style="margin: 0.1em;" height=190
...
- Netflow collector:
  - receive, intercept, and storeflow records
...
- Variation and versions of Netflow
  ...
  - version 9
    ...
    - using a dynamic dat format w/ templates
  - Flexible Netflow
...
- Types of SNMP manager pools
  - get-request: requesting a specific IOD
  - get_next-request: requesting subsequent info after the initial request
...
- SNMP version 3
  ...
  - addinng authentication based on MD5 or SHA
...
- Demo: config NTP authentication
  - topology: R1 <-> NTP
    ...
    - R1: G0/0 w/ .1
    - NTP: .50
  - NTP settings: NTP Service = On; Authentication = Enable; Key = 1; Password = cisco
...
- Dynamic Trunking Protocol (DTP)
  ...
  - dynamic desirable mode: working actively to initate a trunk
...
- Demo: define VRF instance on R1 & R2
  - config VRF instance for Customer A: `R1(config)# ip vrf CUST-A`
  - config VRF instance for Customer B: `R1(config-vrf)# ip vrf CUST-B`
  - verify global routing table but empty: `R1# sh ip route`
  - verify routing table for CUST-A but empty as well: `R1# sh ip vrf CUST-A`
  - verify routing table for CUST-B but empty as well: `R1# sh ip vrf CUST-B`
  ...
  - enter intf Gi0/1: `R1(config-if)# int gig 0/1`
  ...
  - verify connectivity w/ global routing table: `R1# ping 10.10.01.1` $\to$ `..... Success rate is 0 percentage (0/5)`
...
- Demo: using sub-intf to separate traffic btw R1 & R2
  - tasks
  ...
  - assign the sub-intf to CUST-A: `R1(config-subif)# ip vrf forwarding CUST-A`
  ...
  - config another sub-intf for CUST-B:
    ...
    R1(config-subif)# ip addr 30.30.30.2 255.255.255.252
...
- Demo: config routing protocol
  ...
  - OSPF routing protocol required for other routers
    ...
    - R5: `R5(config-router)# network 20.20.20.0 0.0.0.255 area 1`
    - R4: `R4(config-router)# network 10.10.10.0 0.0.0.255 area 1`
    - R5: `R6(config-router)# network 20.20.20.0 0.0.0.255 area 1`

  - config OSPF on R1 w/ both VRF instances
    ...
    R1(config-touter)# router ospf 1 vrf CUST-B
    R1(config-router)# network 10.10.10.0 0.0.0.255 area 1
    R1(config-router)# network 30.30.30.0 0.0.0.3 area 1
    ...
    R1# sh run
    ...
    router ospf1 vrf CUST-A
     network 10.10.10.0 0.0.0.255 area 1
     network 30.30.30.0 0.0.0.3 area 1
    !
    router ospf1 vrf CUST-B
     network 10.10.10.0 0.0.0.255 area 1
     network 30.30.30.0 0.0.0.3 area 1
    ```
  ...
  - config OSPF on R2  w/ similar config
    ...
    R2(config-touter)# router ospf 1 vrf CUST-B
    R2(config-router)# network 20.20.20.0 0.0.0.255 area 1
    R2(config-router)# network 30.30.30.0 0.0.0.3 area 1
    ```
