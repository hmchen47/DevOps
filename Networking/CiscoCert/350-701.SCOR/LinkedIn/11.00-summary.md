...
- Demo: config IKEv1 phase 2 on R1
...
- Demo: bind isakmp & ipsec and apply to intf
...
- Demo: config IPsec on R2
  ...
  ```bash
  ! IKEv1 phase 2: config IPsec transform set
  ...
  ```bash
  ! config ro bin ipsec & isakmp then apply to intf
  R2# conf t
